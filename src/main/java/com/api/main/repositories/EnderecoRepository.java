...
public interface EnderecoRepository extends JpaRepository<EnderecoModel, Long> {
	...
	/**
	 * Cria uma string em formato JSON. Também verifica de o Estado está
	 * relacionado, se não seta null no atributo estado.
	 * 
	 * @param keyword
	 * @return
	 */

	@Query("SELECT " + "CONCAT('{', '\"endereco\"', ':', '{', " + "'\"id\"', ':', e.id, ',', "
			+ "'\"logradouro\"', ':','\"', COALESCE(e.logradouro, ''),'\"', ',', "
			+ "'\"cidade\"', ':','\"', COALESCE(e.cidade, ''),'\"', ',', "
			+ "'\"bairro\"', ':','\"', COALESCE(e.bairro, ''),'\"', ',', "
			+ "'\"cep\"', ':','\"', COALESCE(e.cep, ''),'\"', ',', " + "'\"estado\"', ':', "
			+ "CASE WHEN es.id IS NOT NULL "
			+ "THEN CONCAT('{', '\"id\"', ':', es.id, ',', '\"descricao\"', ':', '\"', es.descricao, '\"', '}') "
			+ "ELSE 'null' " + "END " + ",'}}') " + "FROM EnderecoModel e " + "LEFT JOIN e.estado es "
